...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'pirojodam@gmail.com'; // Tu dirección de correo electrónico de Gmail
        $mail->Password = 'tdty vtqs svke tugy'; // Tu contraseña de Gmail
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;

        // Configurar el remitente y los destinatarios
        $mail->setFrom($correo, $nombre); // Correo y nombre del remitente
        $mail->addAddress('pirojodam@gmail.com'); // Correo del destinatario
        $mail->addAddress($correo); // Agregar al remitente como destinatario adicional

        // Establecer el asunto del correo electrónico
        $asunto = "Nuevo mensaje de formulario de contacto Asunto: " . $_POST['asunto'];
        $mail->Subject = $asunto;
